...
class ClassBalancedSelfTargetTrainer():
    def __init__(self, encoder, classifier, device, args):
        self.device = device
    ...
    def _adapt_train_epoch(self, encoder, classifier, tgt_train_loader, optimizer, reg_weight, buffer=None, replay_coeff=0.1):
        encoder.train()
        classifier.train()
        len_dataloader = len(tgt_train_loader)
        ...
        for i in tqdm(range(len_dataloader)):
            ...
            loss = tgt_loss + replay_loss * replay_coeff
        ...
        total_loss = (total_tgt_loss + total_replay_loss) / (total_tgt_num + total_replay_num)
        total_tgt_loss /= total_tgt_num
        if self.replay:
            total_replay_loss /= total_replay_num

        return total_loss, total_tgt_loss, total_replay_loss
    ...
    def _adapt_test_epoch(self, encoder, classifier, tgt_val_loader, reg_weight):
        encoder.eval()
        classifier.eval()
        len_dataloader = len(tgt_val_loader)
        ...
        total_loss = total_tgt_loss / total_tgt_num
        total_tgt_loss /= total_tgt_num
        tgt_logits = torch.cat(tgt_logits)

        return total_loss, total_tgt_loss, tgt_logits
    ...
    def _adapt_train_test(self, tgt_train_loader, tgt_val_loader, reg_weight, args, p_min=0.2, p_max=0.5, p_inc=0.05, test_epoch_fn=None):
        ...
        for p in p_list:
            ...
            for e in range(1, args.adapt_epochs + 1):
                total_e += 1
                train_loss, train_tgt_loss, train_replay_loss = self._adapt_train_epoch(encoder, classifier, tgt_pseudo_train_loader, optimizer, reg_weight, buffer=buffer)
                val_loss, val_tgt_loss, tgt_logits = self._adapt_test_epoch(encoder, classifier, tgt_pseudo_val_loader, reg_weight)
                ...
                print(
                    f'Reg Weight {reg_weight} p: {round(p,2)} Epoch {total_e} Train Loss: {round(train_loss, 3)} Train Tgt Loss: {round(train_tgt_loss, 3)} Train Replay Loss: {round(train_replay_loss, 3)} \n \
                                Val Total Loss: {round(val_loss, 3)} Val Tgt Loss: {round(val_tgt_loss, 3)}')
